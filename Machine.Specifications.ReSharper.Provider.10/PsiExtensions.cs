...
namespace Machine.Specifications.ReSharperProvider
{
    internal static class PsiExtensions
    {
        ...
        {
            return type.GetAttributeInstances(true)
                .Select(a => new {a, h = new[] {a.GetAttributeType()}.Concat(a.GetAttributeType().GetSuperTypes())})
                .Where(x => x.h.Any(t => t.GetClrName().FullName == FullNames.SubjectAttribute))
                .Select(x => x.a)
                .FirstOrDefault();
        }
        ...
        static IDeclaredType GetValidatedFieldType(this IDeclaredElement element)
        {
            ...
            if (!field.IsValid() || !fieldType.IsResolved)
                return null;
        ...
        public static bool IsContextBaseClass(this IDeclaredElement element)
        {
            ...
            finder.FindInheritors(clazz, searchDomain, findResult.Consumer, NullProgressIndicator.Create());

            return findResult.Found;
        }

        private class InheritedContextFinder
        {
